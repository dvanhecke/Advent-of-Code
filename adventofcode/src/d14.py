...
def part1(bots, size: tuple[int], offset=100):
    ...
    q1 = sum(grid[y][x] for y in range(0, h // 2) for x in range(0, w // 2) if x != w // 2 and y != h // 2)
    q2 = sum(grid[y][x] for y in range(0, h // 2) for x in range(w // 2, w) if x != w // 2 and y != h // 2)
    q3 = sum(grid[y][x] for y in range(h // 2, h) for x in range(0, w // 2) if x != w // 2 and y != h // 2)
    q4 = sum(grid[y][x] for y in range(h // 2, h) for x in range(w // 2, w) if x != w // 2 and y != h // 2)
...
def run(data: str, test = False) -> Solution:
    ...
    return (part1(bots, (11, 7) if test else (101, 103)), part2(bots, (11, 7) if test else (101, 103)))
