...
def get_user_session_id() -> str:
    ...
    if os.path.isfile(USER_SESSION_ID_FILE_PATH):
        ...
        print(
            "Your id is the value of the cookie named",
            highlight("session"),
            f"on {highlight(AOC_URL)}.",
        )
        while not done:
            ...
            if re.fullmatch(r"[\da-f]+", user_session_id):
                ...
                print(
                    'That"s not a valid id. It should consist only of digits and lowercase letters a-f. Please try again.'
                )
    ...
    while result is None:
        try:
            ...
            if response.ok:
                ...
                print(
                    highlight("Success.", color="g"),
                    "Input downloaded and saved to",
                    f"{highlight(input_file_path)}.",
                )
                result = True
            else:
                print(
                    "Server responded with a non-200 status code:",
                    f"{highlight(response.status_code, color="y")}.",
                )
                print("Aborting.")
                result = False
